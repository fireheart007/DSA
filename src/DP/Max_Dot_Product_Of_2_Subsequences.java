...
//     if(i==a.length || j==b.length)return 0; ke badle if(i==a.length || j==b.length)return -(int)1e9;
//     and Math.max(sp1,Math.max(sp2,sp3)) ke badle Math.max(sp1,Math.max(sp2,Math.max(sp3,a[i]*b[j])))
//     isliye kia gya hai kyunki agar a[i]*b[j] -ve hota to pehle vaali statement max(sp1,sp2,sp3) 0 return krti isliye if condition me -(int)1e9 daala or
